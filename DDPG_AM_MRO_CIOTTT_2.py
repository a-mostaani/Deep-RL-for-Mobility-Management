...
class RlAgent:
    def __init__(self):
        ...
        self.path_to_cfg_json = 'C:/Users/Administrator/OneDrive - Nokia/Documents/Software/Season-2020d/SEASON II 2020d/SON_v2/config/scenario_config' \
                                '/Helsinki_MRO_test_AM.json'  # changed-1
        ...
        self.action_dim = 2*(len(self.nbr_cells)+1) + 1  # changed-1
        self.action_bound = 1
    ...
    def next_config(self, timestamp=None):
        ...
        if self.buffer.size() > self.num_explore:
            action = self.actor.predict(self.get_ddpg_state(self.rep_state[-1]))
        else:
        ...
        act_count = 1
        for i in range(len(self.current_cfg.dict_cio_values)):
            for j in range(len(self.current_cfg.dict_cio_values[i])):
                self.current_cfg.dict_cio_values[i][j]['offset'] = float(tf.clip_by_value(action[act_count+1], self.min_CIO, self.max_CIO))
