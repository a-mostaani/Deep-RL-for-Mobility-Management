...
class ReplayBuffer(object):
    ...
    def load(self, save_folder, size=-1):
        ...
        #reward_buffer = np.load(f"{save_folder}")
        ...
        self.reward[:self.size] = reward_buffer[:self.size].reshape((self.size, 1))
        self.not_done[:self.size] = np.ones((self.size, 1)).reshape((self.size, 1))