...
def train_BCQ(state_dim, action_dim, max_action, device, args):
    ...
    while training_iters < args.max_timesteps:
        ...
        if training_iters >= args.max_timesteps:
            torch.save(policy.actor, 'trained_actor_na')
            torch.save(policy.critic, 'trained_critic_na')
            torch.save(policy.vae, 'trained_vae_na')
...
class InputObject:
    def __init__(self, buffer_name):
        infile = open(buffer_name, 'rb')
        self.dict = pickle.load(infile)
        infile.close()
        self.buffer_name = buffer_name.replace('.pickle', '', 1)
        self.buffer = self.dict[2]
        self.env = ""  # useless
        self.seed = 0
        self.eval_freq = 5e3
        self.max_timesteps = 50000
        self.buffer_size = len(self.buffer)
        self.start_timesteps = 200  # useless
        self.rand_action_p = 0.01  # useless
        self.gaussian_std = 0.01  # useless
        self.batch_size = 30  # mini batch size for the NNs
        self.discount = 0.99
        self.tau = 0.005  # target network update rate
        self.lmbda = 0.75  # Weighting for clipped double Q-learning in BCQ
        self.phi = 0.05
...
if __name__ == "__main__":
    ...
    # infile.close()
    args = InputObject('Helsinki_TD3_GroupMask_site04[0][A]_2021-05-08.pickle')

    state_batch = [args.dict[2][x][0] for x in range(args.buffer_size)]
    action_batch = [args.dict[2][x][1] for x in range(args.buffer_size)]
    reward_batch = [args.dict[2][x][2] for x in range(args.buffer_size)]
    next_state_batch = [args.dict[2][x][3] for x in range(args.buffer_size)]
    ...
    # else:  # the next line should be indented if the if-else structure is activated
