...
class RlAgent:
    def __init__(self):
        ...
        # self.offset_pool = list(np.arange(-7, 7, 2))
        ...
        self.config_cnt = 0
        self.target_cell = {40: 'site33[2][A]'}
        self.save_path = 'sim_results/cfg_kpi_site33-2-A.pickle'
        ...
        while self.flag_run:
            try:
                ...
                if (len(self.agent.reports) % self.agent.num_report_per_config == 0) & (self.agent.config_cnt > 0) \
                        & (self.last_msg == 'report') & (self.agent.config_cnt < self.agent.config_max):
