...
class RlAgent:
    def __init__(self):
        ...
        self.num_test = 50        # 96 days of the testing
        ...
        self.save_path = 'Helsinki_SC-DDPG_040B-05-20.pickle'   # single cell DDPG
    ...
    def add_config(self, dict_cfg):
        ...
        print('[INFO] number of collected configurations: ', len(self.configs))
    ...
    def add_replay_sample(self):
        ...
        '''new_state'''
        next_state = self.get_ddpg_state(self.rep_new_state[-1])
    ...
    def next_config(self, timestamp=None):
        ...
        if self.buffer.size() > self.num_explore:
            stt = self.get_ddpg_state(self.rep_state[-1])
            action = self.actor.get_action(stt)
            print("state:", stt)
            print("state dim:", len(stt))
        ...
        df_ofs_ttt.loc[df_ofs_ttt['src_cell'] == self.src_cell, 'TTT'] = float(np.interp(action[0], (-1, 1), (self.min_TTT, self.max_TTT)))
        print("TTT:", df_ofs_ttt.loc[df_ofs_ttt['src_cell'] == self.src_cell, 'TTT'])
        ...
        for i in range(len(self.current_cfg.dict_cio_values[self.src_cell])):
            ...
            self.current_cfg.dict_cio_values[self.src_cell][i]['offset'] = float(np.interp(action[i+1], (-1, 1), (self.min_CIO, self.max_CIO)))
            print("CIO instance:", self.current_cfg.dict_cio_values[self.src_cell][i]['offset'])
            # action_value[0] is TTT, CIO values starts from action_value[1:]

        ls_cio = list(self.current_cfg.dict_cio_values.values())
        print("CIO values:", ls_cio)
        ...
        while self.flag_run:
            try:
                ...
                if list(dict_msg.keys())[0] == 'kpi_report':
                    ...
                    elif self.agent.cnt_sample % 200 == 0:
