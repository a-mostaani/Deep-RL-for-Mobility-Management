"""Script by QL: v2, in working process...
...
2. run_agent: write message to config or kpi report, get the next config with random values, and send back to season
"""
...
class Configuration:
    ...
    def get_path_values(self, tree, path, indx=0):
        ...
        if indx < len(path):
            ...
            self.ho_offsets.append(int(tree[0]['offset']))
            self.ho_ttt.append(int(tree[0]['TTT']))
...
class RlAgent:
    def __init__(self):
        ...
        self.timestamp = 0
        self.offset_range = [-5, 5]  # HO offset speciﬁes the RSRP offset value in dB
        self.TTT_range = [0, 2] # # time to trigger in seconds
        '''
    ...
    def next_config(self, timestamp):
        ...
        while self.flag_run:
            try:
                ...
                if list(dict_msg.keys())[0] == 'kpi_report':
                    ...
                    print('[INFO] number of collected report: ', len(self.agent.reports))
                    self.report_cnt += 1
                elif list(dict_msg.keys())[0] == 'config':
                    self.agent.add_config(dict_msg['config'])

                if len(self.agent.reports) % 3 == 0:
                    ...
                    # next_config = self.agent.next_config(timestamp=None)
                    # put it in send queue
                    self.send_q.put(next_config)

                if len(self.agent.reports) > self.report_max:
                    self.flag_run = False
