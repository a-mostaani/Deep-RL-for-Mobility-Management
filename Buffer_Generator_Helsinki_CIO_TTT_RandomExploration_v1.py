...
class ReplayBuffer:
    ...
    def __init__(self, capacity=2100):
        ...
        self.min_replay_size = 1500  # minimum replay size for start training   # change
...
class RlAgent:
    def __init__(self):
        ...
        self.num_train = 5      # 96*14 (3 days)two weeks of training    # change
        ...
        self.num_update_model = 24     # 24: no. of samples collected to train model with replay experience
        self.num_update_target = 4*12   # 4*24: no. of samples collected to update the target model
        self.num_train_per_update = 30
        self.num_explore = 1500  # change 96*2 is the original
        self.ls_ep_reward = []
        self.ep_idx = 0
        ...
        self.save_path = 'Helsinki_Exploration_1000sample.pickle'   # single cell DQN
    ...
    def next_config(self, timestamp=None):
        ...
        if self.buffer.size() > self.num_explore:
            ...
            action = [random.uniform(self.min_CIO,self.max_CIO) for _ in range(len(self.nbr_cells))]
            action.insert(0, random.uniform(self.min_TTT, self.max_TTT))
        ...
        while self.flag_run:
            try:
                ...
                if list(dict_msg.keys())[0] == 'kpi_report':
                    ...
                    if self.agent.buffer.min_replay_size >= len(self.agent.reports):
                        next_config = self.agent.next_config()
                        self.send_q.put(next_config)
                    else:
                        self.flag_run = False
                        print(self.agent.buffer.min_replay_size)
                        print(len(self.agent.reports))
